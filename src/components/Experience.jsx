      ...
      <div style={{ maxWidth: '800px', width: '100%', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        {experiences.map((exp, index) => (
          ...
          >
            ...
            <div>
              <h3 className="experience-role" style={{ fontSize: '1.6rem', marginBottom: '0.5rem', color: '#ffffff' }}>{exp.role}</h3>
              <p className="experience-company" style={{ fontSize: '1.1rem', color: '#a0a0a0', marginBottom: '0.2rem' }}>{exp.company}</p>
              <p className="experience-date" style={{ fontSize: '0.9rem', color: index % 2 === 0 ? "#00FFFF" : "#FF00FF", fontWeight: 600, marginBottom: (exp.location || exp.skills) ? '0.5rem' : '0' }}>
                {exp.date}
              </p>
              {exp.location && <p className="experience-location" style={{ fontSize: '0.9rem', color: '#888', marginBottom: '0.2rem' }}>{exp.location}</p>}
              {exp.skills && <p className="experience-skills" style={{ fontSize: '0.9rem', color: '#ccc', marginTop: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
